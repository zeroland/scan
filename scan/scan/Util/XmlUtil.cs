...

{
...

    {
        ...
        /// <returns></returns>
        public static string GetTextByAttribute(string xmlFileName, string xPath, string attributeName,string attributeValue)

        {
...

            {
                ...
                XmlElement xn = xmldocument.DocumentElement;
                XmlNodeList nodes= xn.SelectNodes(xPath);
                if (nodes.Count>0)

                {
                    foreach(XmlNode node in nodes)
                    {
                        if (node.Attributes.Count > 0 && node.Attributes["ID"] != null&&node.Attributes["ID"].Value.Equals(attributeValue))
                        {
